...
const Profile = () => {
    ...
    return (
        <div className="pt-8 flex justify-between">
                ...
                <div className="flex items-center">
                    ...
                    <div className="px-4">
                        <div className="flex flex-col">
                            <span className="text-gray-600 font-medium text-[2.5rem]">{componentContext?.state?.user_data_info?.first_name?.charAt(0).toUpperCase()+componentContext?.state?.user_data_info?.first_name?.slice(1)} {componentContext?.state?.user_data_info?.last_name?.charAt(0).toUpperCase() + componentContext?.state?.user_data_info?.last_name?.slice(1)}</span>
                            <span className="text-gray-600 font-medium text-sm text-right">{componentContext?.state?.user_data_info?.email}</span>  
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div className="flex items-center justify-end pt-4 divide-x divide-gray-300 pb-3">
                        <div className="flex items-center px-3">
                            ...
                            <span className="text-gray-600 font-medium text-sm ml-1">{componentContext?.state?.patient_id}</span>
                        </div>
                        <div className="flex items-center px-3">
                            <svg dataSlot="icon" fill="currentColor" className="w-5 h-5 text-gray-600" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                            ...
                            <span className="text-gray-600 font-medium text-sm ml-1">{formattedBirthDate}</span>
                        </div>
                        ...
                        <div className="flex items-center px-3">
                            ...
                            <span className="text-gray-600 font-medium text-sm ml-1">{componentContext?.state?.patient_hrn}</span>
                        </div>   
                        <div className="flex items-center px-3">
            ...
            <div className="flex items-center">
                <button onClick={() => componentContext?.onClick()} className="bg-slate-500 hover:bg-slate-600 rounded-full p-2 flex items-center text-white">
                    <svg dataSlot="icon" fill="currentColor" className="h-5 w-5 " viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
