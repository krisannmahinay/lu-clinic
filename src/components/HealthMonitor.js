...
const HealthMonitor = ({data, onSetHour, onInputtedRR, onInputtedPR, onInputtedTemp, onAddData, onClearData}) => {
    ...
    const minTime = displayedDateTime === formattedCurrentDate ? currentTime : "00"
        ...
        <div>
            <div className="flex items-center pb-10 px-10">
                <div className="flex flex-col">
                    <label className="text-gray-500 font-bold uppercase text-xs">Date</label>
                    <input type="text" value={displayedDateTime} onChange={handleDateTimeChange} readOnly className="border text-xs border-gray-300 px-3 py-2 focus:border-gray-500 focus:outline-none" />

                    <label className="text-gray-500 font-bold uppercase text-xs">Hour</label>
                    <input type="number" placeholder="Enter hour" value={displayedHour} min="0" max="23"  onChange={handleHourChange} className="border text-xs border-gray-300 px-3 py-2 focus:border-gray-500 focus:outline-none" />
                    
                    <label className="text-gray-500 font-bold uppercase text-xs">Respiratory Rate</label>
                    <input type="number" placeholder="Enter RR" value={respiratoryRate} onChange={handleRespiratoryRateChange} className="border text-xs border-gray-300 px-3 py-2 focus:border-gray-500 focus:outline-none" />
                   
                    <label className="text-gray-500 font-bold uppercase text-xs">Pulse Rate</label>
                    <input type="number" placeholder="Enter PR" value={pulseRate} onChange={handlePulseRateChange} className="border text-xs border-gray-300 px-3 py-2 focus:border-gray-500 focus:outline-none" />
                
                    <label className="text-gray-500 font-bold uppercase text-xs">Temperature</label>
                    <input type="number" placeholder="Enter T" value={temperature} onChange={handleTemperatureChange} className="border text-xs border-gray-300 px-3 py-2 focus:border-gray-500 focus:outline-none" />

                    <div className="flex gap-2">
                        <button onClick={() => handleAddData()} title="Add Day" className="flex justify-center items-center bg-green-600 hover:bg-green-700 text-white text-xs p-1 rounded w-full">
                            <svg dataSlot="icon" fill="none" className="w-5 h-5 mr-1" strokeWidth={1.5} stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                            </svg>
                            Add
                        </button>
                        <button onClick={() => handleClearData()} title="Remove Day" className="flex justify-center items-center bg-red-500 hover:bg-red-600 text-white text-xs p-1 rounded w-full">
                            <svg dataSlot="icon" fill="none" className="w-5 h-5 mr-1" strokeWidth={1.5} stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                ...
                <div className="w-full px-8" style={{position: 'relative', top:0}}>
                    <div style={{ position: 'absolute', top: '3rem', left: '25px', fontSize: '10px', color: 'rgb(107 114 128)', width: '5px', lineHeight: '.5rem', fontWeight: 700}}>RR cpm</div>
                    <div style={{ position: 'absolute', top: '3rem', left: 'calc(4% + 12px)', fontSize: '10px', color: 'rgb(107 114 128)', width: '5px', lineHeight: '.5rem', fontWeight: 700}}>PR cpm</div>
                    <div style={{ position: 'absolute', top: '3rem', left: 'calc(7% + 10px)', fontSize: '10px', color: 'rgb(107 114 128)', width: '5px', lineHeight: '.5rem', fontWeight: 700}}>Temp</div>
